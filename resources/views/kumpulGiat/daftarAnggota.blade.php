    ...
    <main id="main">
        ...
        <div class="formbold-main-wrapper">
            ...
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                <a href="{{ route('veriUser.veriUser') }}" class="btn btn-primary"> <i class="fa fa-plus"></i> Tambah Data
                </a>
            </div>
            <div class="container">
                <div class="card">
                            ...
                            <table class="table table-striped">
                                <thead>
                                    <tr style="text-align: center">
                                        ...
                                        <th scope="col">{{ __('Paramedis') }}</th>
                                        <th scope="col">{{ __('Navigasi') }}</th>
                                        <th scope="col">{{ __('Evakuasi') }}</th>
                                        <th scope="col">{{ __('Desk Posko') }}</th>
                                ...
                                <tbody>
                                    @foreach ($user as $user)
                                        <tr style="text-align: center">
                                            <td>{{ $user->name }}</td>
                                            <td>{{ $user->paramedis }}</td>
                                            <td>{{ $user->navigasi }}</td>
                                            <td>{{ $user->evakuasi }}</td>
                                            <td>{{ $user->deskPosko }}</td>
                                            <td>{{ $user->komunikasi }}</td>
                                            <td>{{ $user->kerjaTim }}</td>
                                            <td>
                                                <div class="btn-group" role="group" aria-label="{{ __('User') }}">
                                                    <a href="{{ route('veriUser.edit', $user->id) }}"
                                                        class="btn btn-success"><i class="fa-edit w-100 p-4">Edit</i></a>
                                                    <form action="{{ route('veriUser.destroy', $user->id) }}" method="POST"
